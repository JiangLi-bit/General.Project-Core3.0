...
namespace General.Framework
{
    ...
    public class PermissionHandler : AuthorizationHandler<PermissionRequirement>
    {
        ...
        protected override async Task HandleRequirementAsync(AuthorizationHandlerContext context, PermissionRequirement requirement)
        {
            ...
            {
                //获取 uid 的两种方式
                //var uid = (JwtHelper.SerializeToken(httpContext.Request.Headers["Authorization"].ToStr().Replace("Bearer ", "")))?.Uid;
                var uid = (from item in httpContext.User.Claims where item.Type == "jti" select item.Value).FirstOrDefault().ToStr();
                Guid id = new Guid(uid);

                var data = sysPermissionService.GetPermissionListById(id);
            ...
            if (httpContext != null)
            {
                ...
                if (defaultAuthenticate != null)
                {
                    ...
                    if (result?.Principal != null)
                    {
                        ...
                        var permisssionRoles = requirement.Permissions.Where(w => currentUserRoles.Contains(w.RoleName.ToStr()));
                        foreach (var item in permisssionRoles)
                        {
                            try
                            {
                                if (Regex.Match(questUrl, item.Url?.ToStr().ToLower())?.Value == questUrl)
                                {
                                    isMatchRole = true;
                                    break;
                                }
                        ...
                        //判断过期时间（这里仅仅是最坏验证原则，你可以不要这个if else的判断，因为我们使用的官方验证，
                        //Token过期后上边的result?.Principal 就为 null 了，进不到这里了，因此这里其实可以不用验证过期时间，只是做最后严谨判断）
